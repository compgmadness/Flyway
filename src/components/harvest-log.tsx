...
import { DEFAULT_PLACE, FEATURED_SPOTS, type Place } from "@/lib/flyway/places";
...
export function HarvestLogForm({ bird }: { bird: BirdId }) {
  ...
  const spots = useMemo(() => {
    const rest = FEATURED_SPOTS.filter((s) => s.name !== marsh.name).slice(0, 8);
    return [marsh, ...rest];
  }, [marsh]);
  const [placeName, setPlaceName] = useState(marsh.name);
  ...
  const place = spots.find((s) => s.name === placeName) ?? marsh;
  ...
  function submit() {
    ...
    setSaved(`Logged ${count} ${bird.name.toLowerCase()}${count === 1 ? "" : "s"} at ${place.name}. The brief now uses this bag.`);
    setNote("");
  ...
  return (
    <section className="rounded-2xl bg-surface p-4 shadow-border">
      ...
      <p className="mt-2 text-sm leading-relaxed text-muted">
        Bag ID is the log, not just a key. This sits on the flyway and moves the sit number for
        hunters south of you. Not a legal harvest report.
      </p>
          ...
          >
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-muted">
                {row.place.name} · {ICE_LABEL[row.ice]}
              </p>
            </div>
            <button
              type="button"
